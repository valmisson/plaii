...
from app.data.repositories import PlayerRepository
...
class PlayerBar(Container):
    ...
    def on_settings_folder_subscribe(self, _, data: dict):
        ...
        # Currently only handling remove events
        if state == 'remove':
            self._handle_folder_removal(folder_path)

    def _handle_folder_removal(self, folder_path: str):
        """
        Remove all references to music from a specific folder in the player state and playlist.
        If the currently playing, next, or previous music is from the removed folder, clear them.
        Update the UI and persist changes if any state was modified.

        Args:
            folder_path (str): The path of the folder being removed.
        """
        # Retrieve the latest player state
        player_state = self.player_repository.get_player_state()
        playlist_changed = False
        cleared_current = False

        # Clear current music if it belongs to the removed folder
        current_music = player_state.current_music
        if current_music and current_music.get('folder') == folder_path:
            self.audio_service.reset()
            player_state.current_music = None
            player_state.current_album = None
            player_state.audio_position = 0
            player_state.audio_duration = 0
            self._update_play_button(False)
            self._update_music_info(Music(
                title='Titulo da musica',
                artist='Nome do artista',
                album='Álbum desconhecido',
                album_artist='Artista desconhecido',
                duration=DEFAULT_DURATION_TEXT,
                filename='none'
            ))
            cleared_current = True

        # Clear next/previous music if they belong to the removed folder
        for attr in ['next_music', 'prev_music']:
            music = getattr(player_state, attr)
            if music and music.get('folder') == folder_path:
                setattr(player_state, attr, None)
        ...
        if cleared_current or playlist_changed:
